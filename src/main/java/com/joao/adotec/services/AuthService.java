...
public class AuthService {
    ...
    public UserInfoResponse registerUser(SignupRequest signUpRequest) {
        if (userRepository.existsByEmail(signUpRequest.getEmail())) {
            throw new DomainException(HttpStatus.CONFLICT, "Error: Email is already in use!");
        }
        ...
        Role userRole = roleRepository.findByRoleName(AppRole.ROLE_ADOPTER)
                .orElseThrow(() -> new DomainException(HttpStatus.INTERNAL_SERVER_ERROR, "Error: Default role 'ADOPTER' is not found."));
        roles.add(userRole);
        user.setRoles(roles);
